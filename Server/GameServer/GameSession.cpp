...
void GameSession::OnConnected()
{
	...
	// 더미 접속 시 임시 ID 발급 (실제로는 로그인 패킷에서 계정 ID를 받아서 처리해야 함)
...
JobTask GameSession::LoadPlayerTask(uint64 accountId, uint64 playerId)
{
	...
	std::shared_ptr<PlayerSaveData> loadedData = std::make_shared<PlayerSaveData>();
	loadedData->playerId = playerId;

	auto dbJob = [playerId, loadedData](DBConnection* conn) {
		// Mock: 실제로는 SELECT 쿼리를 날려 값을 가져옵니다.
		// std::string query = "SELECT Gold, PosX, PosY, PosZ FROM Player WHERE PlayerId = " + std::to_string(playerId);
		// conn->Execute(query);
	...
	player->GetPlayerInfo()->set_name(loadedData->name);
	player->GetPlayerInfo()->mutable_posinfo()->set_x(loadedData->x);
	player->GetPlayerInfo()->mutable_posinfo()->set_y(loadedData->y);
	player->GetPlayerInfo()->mutable_posinfo()->set_z(loadedData->z);
