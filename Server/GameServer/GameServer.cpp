 ...
 * 데이터 흐름: 종료 시그널 발생 -> GIsRunning 플래그 false로 변경 -> 메인 틱 루프 탈출 -> DB 강제 플러시 -> 안전 종료
 */
BOOL WINAPI ConsoleCtrlHandler(DWORD dwCtrlType)
{
	...
	case CTRL_BREAK_EVENT:
		std::cout << "\n[System] Shutdown signal received. Preparing graceful shutdown..." << std::endl;
		GIsRunning = false;
...
int main()
{
	...
	std::cout << "GameServer is running on port " << GConfigManager->serverConfig.port << "..." << std::endl;
	...
	std::cout << "[System] Main loop exited. Broadcasting AutoSave to all rooms..." << std::endl;
	
	// Phase 3: Graceful Shutdown
	// 모든 접속 중인 유저들의 상태를 캡처하여 DB에 스냅샷 저장(AutoSave)을 명령합니다.
	GGameRoomManager->BroadcastAutoSave();

	// 약간의 딜레이를 주어 GameRoom 워커 스레드들이 DB 워커(DBConnectionPool 큐)로 작업을 넘길 시간을 줍니다.
	std::this_thread::sleep_for(std::chrono::seconds(1));

	std::cout << "[System] Waiting for worker threads and DB flush..." << std::endl;

	// GThreadManager->Join()을 통해 모든 워커 스레드(DB 포함)가 큐에 쌓인 남은 작업을
	// 모두 소진(Flush)할 때까지 대기한 후 안전하게 스레드들을 회수합니다.
	GThreadManager->Join();
	SocketUtils::Clear();
	return 0;
}
